...
class Plot():
    def __init__(self):
        self.api_key = '7P0IOH3LDW52HMLH'

    def obter_dados_alpha_vantage(self, endpoint, params):
    ...
    def obter_dados_acao(self, ticker, interval='daily', outputsize='full'):
        endpoint = '/query'
        params = {
        ...
        data = self.obter_dados_alpha_vantage(endpoint, params)
        if data:
            key = 'Time Series (Daily)' if interval == 'daily' else 'Time Series (1min)'
            return pd.DataFrame(data[key]).T.reset_index().rename(columns={'index': 'date'})

        return None

    def full_plot(self, ticker):
        # Choose Ticker, Start Date, and period
        ...
        data_daily, meta_data_ts = self.ts.get_daily(ticker, outputsize='full')  # pylint: disable=unbalanced-tuple-unpacking
        ...
        plotting_data = [trace_close, trace_candles, trace_bars, trace_arrow, trace_ema8, trace_ema21, trace_sma, trace_volume,
    ...
    def plot_graphic(self, ticker):
        self.full_plot(ticker)

if __name__ == '__main__':
    plot = Plot()
    plot.plot_graphic('AAPL')