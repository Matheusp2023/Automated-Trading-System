...
class Login():
    ...
    def login_window_screen(self):
        ...
        label_tt = ctk.CTkLabel(master=window, text="Entre na sua conta e acesse\n a plataforma!", font=("Roboto", 18), text_color="#00B0F0").place(x=60, y=10)

        login_frame = ctk.CTkFrame(master=window, width=350, height=396)
        login_frame.pack(side=RIGHT)

        label = ctk.CTkLabel(master=login_frame, text="Sistema de Login", font=("Roboto", 20), text_color=("black", "white")).place(x=25, y=5)
        ...
        username_label = ctk.CTkLabel(master=login_frame, text="*O campo nome de usuário é obrigatório.", text_color="green", font=("Roboto",8)).place(x=25, y=135)

        password_entry = ctk.CTkEntry(master=login_frame, placeholder_text="Senha do usuário", width=300, font=("Roboto", 14), show="*").place(x=25, y=175)
        password_label = ctk.CTkLabel(master=login_frame, text="*O campo senha do usuário é obrigatório.", text_color="green", font=("Roboto",8)).place(x=25, y=205)

        password_checkbox = ctk.CTkCheckBox(master=login_frame, text="Lembrar da minha senha").place(x=25, y=235)
        ...
        def screen_register():
            ...
            password = ctk.CTkEntry(master=rg_frame, placeholder_text="Senha de usuário", width=300, font=("Roboto", 14), show="*").place(x=25, y=185)
            confirm_password = ctk.CTkEntry(master=rg_frame, placeholder_text="Confirmar senha", width=300, font=("Roboto", 14), show="*").place(x=25, y=225)

            usuary_terms_checkbox = ctk.CTkCheckBox(master=rg_frame, text="Eu concordo com os termos de usuário").place(x=25, y=265)
        ...
        register_button = ctk.CTkButton(master=login_frame, text="Cadastre-se", width=169, fg_color="green", hover_color="#2D9334", command=screen_register).place(x=156, y=325)
